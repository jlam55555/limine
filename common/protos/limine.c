...
static pagemap_t build_pagemap(int paging_mode, bool nx,
                               struct elf_range *ranges, size_t ranges_count,
  ...
  for (size_t i = 0; i < _memmap_entries; i++) {
    ...
    for (uint64_t j = 0; j < aligned_length; j += 0x40000000) {
      ...
      map_page(pagemap, direct_map_offset + page, page, VMM_FLAG_WRITE,
               Size1GiB);
    }
  }
  ...
  for (size_t i = 0; i < _memmap_entries; i++) {
    ...
    for (uint64_t j = 0; j < aligned_length; j += 0x1000) {
      uint64_t page = aligned_base + j;
      map_page(pagemap, direct_map_offset + page, page,
               VMM_FLAG_WRITE | VMM_FLAG_FB, Size4KiB);
    }
  }
...
#define LIMINE_MAIR(fb)                                                        \
  (((uint64_t)0b11111111 << 0) /* Normal WB RW-allocate non-transient */       \
   | ((uint64_t)(fb) << 8))    /* Framebuffer type */

#define LIMINE_TCR(tsz, pa)                                                    \
  (((uint64_t)(pa) << 32) /* Intermediate address size */                      \
   ...
   |                                                                           \
   ((uint64_t)(tsz) << 16) /* Address bits in TTBR1 */ /* TTBR0 4K granule */  \
...
noreturn void limine_load(char *config, char *cmdline) {
  ...
  if ((kernel_file = uri_open(kernel_path)) == NULL)
    panic(true,
          "limine: Failed to open kernel with path `%#`. Is the path correct?",
          kernel_path);
  ...
  if (!elf64_load(kernel, &entry_point, &slide, MEMMAP_KERNEL_AND_MODULES,
                  kaslr, &ranges, &ranges_count, &physical_base, &virtual_base,
                  NULL, &image_size_before_bss, &is_reloc)) {
  ...
  if (elf64_load_section(kernel, &requests, ".limine_reqs", 0, slide)) {
    ...
    for (size_t i = 0; i < ALIGN_DOWN(image_size_before_bss, 8); i += 8) {
      ...
      if (_get_request(p) != NULL) {
        panic(true, "limine: Conflict detected for request ID %X %X", p[2],
              p[3]);
      }
  ...
  paging_mode = max_paging_mode >= PAGING_MODE_RISCV_SV48
                    ? PAGING_MODE_RISCV_SV48
                    : PAGING_MODE_RISCV_SV39;
...
#define paging_mode_vmm_to_limine(x) (x)
  ...
  FEAT_START
  struct limine_paging_mode_request *pm_request =
      get_request(LIMINE_PAGING_MODE_REQUEST);
  if (pm_request == NULL)
    break;
  ...
  *kf = get_file(kernel_file, cmdline);
  ...
  // Entry point feature
  FEAT_START
  struct limine_entry_point_request *entrypoint_request =
      get_request(LIMINE_ENTRY_POINT_REQUEST);
  ...
  struct limine_bootloader_info_request *bootloader_info_request =
      get_request(LIMINE_BOOTLOADER_INFO_REQUEST);
  ...
  struct limine_kernel_address_request *kernel_address_request =
      get_request(LIMINE_KERNEL_ADDRESS_REQUEST);
  ...
  struct limine_smbios_request *smbios_request =
      get_request(LIMINE_SMBIOS_REQUEST);
  ...
  struct limine_efi_system_table_request *est_request =
      get_request(LIMINE_EFI_SYSTEM_TABLE_REQUEST);
  if (est_request == NULL) {
    break; // next feature
  }
...
#if defined(UEFI)
  // TODO: Looking for the DTB should be moved out of here and into lib/,
  // because:
  // 1. We will need it for core bring-up for the SMP request.
  // 2. We will need to patch it for the Linux boot protocol to set the
  // initramfs
  //    and boot arguments.
  // 3. If Limine is ported to platforms that use a DTB but do not use UEFI, it
  // will
  ...
  struct limine_stack_size_request *stack_size_request =
      get_request(LIMINE_STACK_SIZE_REQUEST);
  ...
  struct limine_kernel_file_request *kernel_file_request =
      get_request(LIMINE_KERNEL_FILE_REQUEST);
  ...
  struct limine_module_request *module_request =
      get_request(LIMINE_MODULE_REQUEST);
  ...
  struct limine_file *modules =
      ext_mem_alloc(module_count * sizeof(struct limine_file));
  ...
  for (size_t i = 0; i < module_count; i++) {
    ...
    if (module_request->revision >= 1 &&
        i < module_request->internal_module_count) {
      uint64_t *internal_modules =
          (void *)get_phys_addr(module_request->internal_modules);
      struct limine_internal_module *internal_module =
          (void *)get_phys_addr(internal_modules[i]);
      ...
      module_required =
          internal_module->flags & LIMINE_INTERNAL_MODULE_REQUIRED;
    } else {
      struct conf_tuple conf_tuple =
          config_get_tuple(config,
                           i - (module_request->revision >= 1
    ...
    if ((f = uri_open(module_path)) == NULL) {
      if (module_required) {
        panic(true,
              "limine: Failed to open module with path `%#`. Is the path "
              "correct?",
              module_path);
      }
...
skip_fb_init:
  for (size_t i = 0; i < fbs_count; i++) {
    memmap_alloc_range(fbs[i].framebuffer_addr,
                       (uint64_t)fbs[i].framebuffer_pitch *
                           fbs[i].framebuffer_height,
  ...
  struct limine_framebuffer_request *framebuffer_request =
      get_request(LIMINE_FRAMEBUFFER_REQUEST);
  if (framebuffer_request == NULL) {
    break; // next feature
  }

  struct limine_framebuffer *fbp =
      ext_mem_alloc(fbs_count * sizeof(struct limine_framebuffer));
...
no_fb:
  ...
  struct limine_boot_time_request *boot_time_request =
      get_request(LIMINE_BOOT_TIME_REQUEST);
  ...
  for (size_t i = 0; i < fbs_count; i++) {
    ...
    // Prefer tighter Device memory (lower values)
             ...
             fb_attr > new_attr)
      fb_attr = new_attr;
    // Use Normal non-cacheable otherwise (avoid trying to figure out how to
    // downgrade inner vs outer).
    else if ((fb_attr & 0b11110000) && (new_attr & 0b11110000))
  ...
  if (riscv_boot_proto != NULL) {
    if (riscv_boot_proto->GetBootHartId(riscv_boot_proto, &bsp_hartid) ==
        EFI_SUCCESS) {
  ...
  smp_info = init_smp(&cpu_count, &bsp_lapic_id, true, paging_mode, pagemap,
                      smp_request->flags & LIMINE_SMP_X2APIC, nx_available,
                      direct_map_offset, true);
#elif defined(__aarch64__)
  uint64_t bsp_mpidr;

  smp_info = init_smp(&cpu_count, &bsp_mpidr, pagemap, LIMINE_MAIR(fb_attr),
                      LIMINE_TCR(tsz, pa), LIMINE_SCTLR, direct_map_offset);
#elif defined(__riscv64)
  if (!have_bsp_hartid) {
...
#if defined(__x86_64__) || defined(__i386__)
  smp_response->flags |=
      (smp_request->flags & LIMINE_SMP_X2APIC) && x2apic_check();
...
#error Unknown architecture
  ...
  struct limine_memmap_request *memmap_request =
      get_request(LIMINE_MEMMAP_REQUEST);
  ...
  common_spinup(
      limine_spinup_32, 10, paging_mode, (uint32_t)(uintptr_t)pagemap.top_level,
      (uint32_t)entry_point, (uint32_t)(entry_point >> 32),
      (uint32_t)reported_stack, (uint32_t)(reported_stack >> 32),
      (uint32_t)(uintptr_t)local_gdt, nx_available, (uint32_t)direct_map_offset,
      (uint32_t)(direct_map_offset >> 32));
#elif defined(__aarch64__)
  vmm_assert_4k_pages();

  uint64_t reported_stack = reported_addr(stack);

  enter_in_el1(entry_point, reported_stack, LIMINE_SCTLR, LIMINE_MAIR(fb_attr),
               LIMINE_TCR(tsz, pa), (uint64_t)pagemap.top_level[0],
               (uint64_t)pagemap.top_level[1], direct_map_offset);
#elif defined(__riscv64)
  uint64_t reported_stack = reported_addr(stack);
